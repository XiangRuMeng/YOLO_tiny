...
class Resblock_body(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)  # 利用一个3x3卷积进行特征整合
        route = x  # 引出一个大的残差边route
        c = self.out_channels
        x = torch.split(x, c//2, dim = 1)[1] # 对特征层的通道进行分割，取第二部分作为主干部分。
        x = self.conv2(x) # 对主干部分进行3x3卷积
        route1 = x # 引出一个小的残差边route_1
        x = self.conv3(x) # 对第主干部分进行3x3卷积
        x = torch.cat([x,route1], dim = 1)  # 主干部分与残差部分进行相接
        x = self.conv4(x) # 对相接后的结果进行1x1卷积
        feat = x
        x = torch.cat([route, x], dim = 1)
        x = self.maxpool(x) # 利用最大池化进行高和宽的压缩
        return x,feat
